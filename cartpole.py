...
def _loss(state, sig):
    if sig is None:
        sig = np.full_like(state, 0.5)  # default sigma = 0.5

    state = np.asarray(state)
    sig = np.asarray(sig)

    if not np.all(np.isfinite(sig)) or np.any(sig == 0):
        raise ValueError("All sigma values must be finite and non-zero")

    exponent = -np.sum((state**2) / (2.0 * sig**2))
    return 1 - np.exp(exponent)

def loss(state, sig=None):
    return _loss(state, sig=sig)
...
class CartPole:
    ...
    def __init__(self, visual=False):
        ...
        self.sim_steps = 1         # number of Euler integration steps to perform in one go
    ...
    def loss(self, sig=None):
        return _loss(self.getState(), sig=sig)
